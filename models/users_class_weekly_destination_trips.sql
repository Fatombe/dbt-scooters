with
      ...
      morning_trips - the number of morning trips */
        select
            ...
            count(
                case
                    when
                        extract(hour from started_at at time zone 'Europe/Moscow')
                        between 6 and 10
                    then 1
                end
            ) as morning_trips
    ...
    and the trip statistics to those destinations:
      avg_morning_trip_days - the average number of days with morning trips per week */
        select user_id, destination, avg(morning_trip_days) as avg_morning_trip_days
        from weekly_stat_cte
        group by 1, 2
    )
select user_id, max(avg_morning_trip_days) >= 3 as to_work
